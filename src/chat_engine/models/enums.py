...
class PreferenceType(Enum):
    ...
    HANDICAP_ACCESSIBLE = (
        "Accessibility",
        "Handicap accessible",
        "Parking lot is accessible for handicapped individuals.",
    )
    ...
    COVERED_PARKING = (
        "Features",
        "Covered parking",
        "Parking lot has covered parking spaces to protect vehicles from weather conditions.",
    )
    ...
    LONG_TERM_PARKING = (
        "Features",
        "Long-term parking",
        "Parking lot is ideal for long-term stays. It is located farther from the destination but cost-effective,"
        " convenient and they are often with secure/surveillance features.",
    )
    SHORT_TERM_PARKING = (
        "Features",
        "Short-term parking",
        "Parking lot ideal for quick trips, pickups. It is located near terminals."
        " Cheap for short stays but expensive for long stays.",
    )
    VALET_SERVICE = (
        "Features",
        "Valet service",
        "A convenient amenity where an attendant parks and retrieves the guest's vehicle.",
    )
    ...
    def __refr__(self):
        return f"{self.category} | {self.value} - {self.description}"

    def to_dict(self):
        """Convert the PreferenceType enum member to a dictionary."""
        return {"category": self.category, "type": self.value, "description": self.description}
